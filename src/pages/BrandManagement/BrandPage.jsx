...
export default function BrandPage() {
  const { brands, deleteBrand } = UseBrandPage();
  ...
  const filteredBrands = brands.filter((emp) =>
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <h1 className="text-md font-bold text-gray-800"> Brand Management</h1>

      {isAdding ? (
        <AddBrandPage onClose={() => setIsAdding(false)} />
      ) : editData ? (
        <EditBrandPage data={editData} onClose={() => setEditData(null)} />
      ) : (
        <>
          {/* Filter Header */}
<motion.div
