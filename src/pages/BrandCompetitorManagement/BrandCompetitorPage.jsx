...
import AddBrandCompetitorPage from "./AddBrandCompetitorPage";
import EditBrandCompetitorPage from "./EditBrandCompetitorPage";
import UseBrandCompetitorPage from "./UseBrandCompetitorPage";
import { FaPlus, FaUpload, FaDownload, FaEye } from "react-icons/fa";

export default function BrandCompetitorPage() {
  const { brandcompetitors, deleteBrandCompetitor } = UseBrandCompetitorPage();
  ...
  const filteredBrandCompetitors = brandcompetitors.filter((emp) =>
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <h1 className="text-md font-bold text-gray-800"> Brand Competitor Management</h1>

      {isAdding ? (
        <AddBrandCompetitorPage onClose={() => setIsAdding(false)} />
      ) : editData ? (
        <EditBrandCompetitorPage data={editData} onClose={() => setEditData(null)} />
      ) : (
        <>
          {/* Filter Header */}
<motion.div
      ...
      >
        <div className="bg-white p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <input
            ...
            value={filters.brandcompetitor}
            onChange={handleFilterChange}
            placeholder="BrandCompetitor"
          ...
          >
            <BrandCompetitorTable
              data={filteredBrandCompetitors}
              onEdit={(row) => setEditData(row)}
              onDelete={deleteBrandCompetitor}
            />
          </motion.div>
        </>
      )}
