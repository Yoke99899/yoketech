...
export default function CategoryPage() {
  const { categorys, deleteCategory } = UseCategoryPage();
  ...
  const filteredCategorys = categorys.filter((emp) =>
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <h1 className="text-md font-bold text-gray-800"> Category Management</h1>

      {isAdding ? (
        <AddCategoryPage onClose={() => setIsAdding(false)} />
      ) : editData ? (
        <EditCategoryPage data={editData} onClose={() => setEditData(null)} />
      ) : (
        <>
          {/* Filter Header */}
<motion.div
