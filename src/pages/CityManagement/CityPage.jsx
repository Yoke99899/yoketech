...
export default function CityPage() {
  const { citys, deleteCity } = UseCityPage();
  ...
  const filteredCitys = citys.filter((emp) =>
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <h1 className="text-md font-bold text-gray-800"> City Management</h1>

      {isAdding ? (
        <AddCityPage onClose={() => setIsAdding(false)} />
      ) : editData ? (
        <EditCityPage data={editData} onClose={() => setEditData(null)} />
      ) : (
        <>
          {/* Filter Header */}
<motion.div
