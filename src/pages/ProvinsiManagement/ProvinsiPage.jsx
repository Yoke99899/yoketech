...
import AddProvinsiPage from "./AddProvinsiPage";
import EditProvinsiPage from "./EditProvinsiPage";
...
export default function ProvinsiPage() {
  const { provinsis, deleteProvinsi } = UseProvinsiPage();
  ...
  const filteredProvinsis = provinsis.filter((emp) =>
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      <h1 className="text-md font-bold text-gray-800"> Provinsi Management</h1>

      {isAdding ? (
        <AddProvinsiPage onClose={() => setIsAdding(false)} />
      ) : editData ? (
        <EditProvinsiPage data={editData} onClose={() => setEditData(null)} />
      ) : (
        <>
          {/* Filter Header */}
<motion.div
    ...
    {showFilter && (
      ...
      >
        <div className="bg-white p-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          >
            <ProvinsiTable
              data={filteredProvinsis}
              onEdit={(row) => setEditData(row)}
              onDelete={deleteProvinsi}
            />
          </motion.div>
        </>
      )}
