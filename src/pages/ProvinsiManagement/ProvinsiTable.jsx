...
export default function ProvinsiTable({ data, onEdit, onDelete }) {
      ...
      <table className="w-full table-auto text-xs text-gray-800">
        <thead className="bg-gray-800 text-white sticky top-0 z-10">
          <tr>
            ...
            <th className="p-2 border whitespace-nowrap">Provinsi</th>
            <th className="p-2 border whitespace-nowrap">Aksi</th>
          </tr>
        </thead>
        <tbody>
          {data.map((provinsis, idx) => (
            <tr key={provinsis.id || idx} className="hover:bg-gray-50">
              <td className="p-2 border text-center">{idx + 1}</td>
              <td className="p-2 border">{provinsis.provinsi}</td>
              <td className="p-2 border text-center">
                <div className="flex justify-center gap-2">
                  <button
                    onClick={() => onEdit(provinsis)}
                    className="px-2 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded text-xs transition"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => handleConfirm(provinsis.id)}
